...
function createWaveBlob(track, options) {
    ...
    var melodyKind = track.bpm > 110 ? 'soft-square' : 'triangle';
    ...
    for (var sampleIndex = 0; sampleIndex < sampleCount; sampleIndex++) {
        ...
        value += oscillator(2 * Math.PI * midiFrequency(melodyNote) * time, melodyKind) * 0.17 * noteEnvelope;

        var beatPhase = beat % 1;
        value += Math.sin(2 * Math.PI * (72 - beatPhase * 34) * time) * Math.exp(-beatPhase * 11) * (track.bpm > 90 ? 0.20 : 0.12);
