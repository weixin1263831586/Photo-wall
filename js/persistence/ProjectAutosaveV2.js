...
function openDatabase(factory, databaseName) {
    return new Promise(function (resolve, reject) {
        ...
        request.onupgradeneeded = function () {
            ...
            if (!database.objectStoreNames.contains(AUDIO_STORE)) database.createObjectStore(AUDIO_STORE, { keyPath: 'id' });
        };
...
/** IndexedDB-backed latest-project snapshot with bounded manual backups. */
export function createProjectAutosave(options) {
    ...
    var mobile = options.mobile === true || (options.mobile !== false && isLikelyMobile());
    var backupLimit = Math.max(1, Math.min(5,
        Number(options.backupLimit) || (mobile ? 2 : 5)));
    var maxBackupBytes = Math.max(16 * 1024 * 1024,
        Number(options.maxBackupBytes) || (mobile ? 256 * 1024 * 1024 : 1024 * 1024 * 1024));
    ...
    async function persist(snapshot) {
        ...
        preparedPhotos.forEach(function (photo) {
            if (previousFingerprints[photo.id] !== photo.fingerprint) photoStore.put({ id: photo.id, blob: photo.blob });
        });
    ...
    async function loadLatest() {
        ...
        var photos = record.project.photos.map(function (metadata, index) {
            ...
            if (!stored || !isBlob(stored.blob)) {
                skippedPhotoCount++;
                return null;
            }
            return Object.assign({}, metadata, { originalBlob: stored.blob, blob: stored.blob });
        }).filter(Boolean);
        if (!photos.length && record.project.photos.length) {
            throw new Error('Autosave photo data is incomplete');
        }
        var music = null;
        if (record.project.backgroundMusic) {
            ...
            if (storedMusic && isBlob(storedMusic.blob)) {
    ...
    function createBackup(label) {
        ...
        return Promise.resolve().then(capture).then(async function (snapshot) {
            ...
            var musicBlob = snapshot.project.backgroundMusic &&
                (snapshot.project.backgroundMusic.originalBlob || snapshot.project.backgroundMusic.blob);
            var backupBytes = preparedPhotos.reduce(function (total, photo) {
                return total + (photo.blob && photo.blob.size ? photo.blob.size : 0);
            }, musicBlob instanceof Blob ? musicBlob.size : 0);
            if (backupBytes > maxBackupBytes) return null;
            ...
            store.put(record);
            while (keys.length >= backupLimit) store.delete(keys.shift());
            await transactionDone(transaction);
    ...
    async function loadBackup(id) {
        ...
        var validPhotos = record.photos.filter(function (photo) { return photo && isBlob(photo.blob); });
        if (!validPhotos.length && record.photos.length) throw new Error('Backup photo data is incomplete');
        return {
            savedAt: record.savedAt,
            label: record.label,
            skippedPhotoCount: record.photos.length - validPhotos.length,
            project: Object.assign({}, record.project, {
                photos: validPhotos.map(function (photo) {
